...
import { listFiles, type FileItem, unstage, type StagedItem } from "@/lib/api.ts";
...
type Props = {
    chatId: number | null;
    draftId: string | null;
    staged?: StagedItem[]; // guard against undefined from parent init
...
export default function AttachmentsShelf({
    ...
    // Load saved files for an existing chat
    React.useEffect(() => {
        let cancelled = false;
        (async () => {
            if (!chatId) {
    ...
    // Nothing to render? Keep behavior consistent.
    if (!showPending && !showSaved) return null;

    return (
        <div
            className="attachments-shelf hidden xl:block"
            style={{
        ...
        >
            {/* We keep LTR on the bubble to make intrinsic width grow toward +X */}
            <div
                dir="ltr"
                className="attachments-bubble rounded-2xl border bg-[var(--panel)] shadow-sm overflow-hidden"
            ...
            >
                ...
                {showPending && (
                    <div className="px-3 py-2 border-b border-[var(--border)]">
                        <div className="text-xs uppercase tracking-wide opacity-70 mb-2 flex items-center gap-1">
                            ...
                            <span>ממתין לשליחת השאילתא</span>
                        </div>
                ...
                <div className="px-3 py-2">
                    ...
                    {!chatId ? (
                        <div className="text-xs opacity-60">התחל שיחה או צרף קובץ</div>
                    ) : showSaved ? (
                        <ul className="grid gap-1.5">
                            {saved.map((f) => (
                                <li
